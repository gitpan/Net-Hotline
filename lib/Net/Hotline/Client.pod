...
object, a download reference number, and the size of the download on success,
or undef if an error occurred.  Those return values are meant to be fed to
C<recv_file()> like this (error handling omitted):
...
contents of the server directory PATH, or undef if an error occurred. 
...
upload reference number, and the size of the upload, or undef if
an error occurred.  Those return values are meant to be fed to
...
needed to resume the upload, or undef if an error occurred. 
Those return values are meant to be fed to C<send_file()> like this (error
handling omitted):
...
undef if there are no matches.  Also returns undef if called before the
userlist has been retrieved from the server. REGEX is treated as a
case-sensitive anchored regular expression internally (i.e. C</^REGEX$/>). 
If your regex matches more than one user's nickname, and C<user_by_nick()>
was called in array context, an array of references to user objects will be
returned. Otherwise, the first user object that matched will be returned
(as ordered by socket number, from low to high).
