...
Net::Hotline::Client is a class implementing a limited Hotline internet
client in Perl.  It was specifically developed to aid in the creation of
Hotline "bots." Hotline is an internet client/server system that's a sort
of cross between IRC and a BBS.  See http://www.hotlinesw.com/ for more
information.
...
developed with the aid of the C source code from the Unix "hx" Hotline
...
data. Some features are not yet implemented, the most notable being file
upload and private chat capabilities.  Finally, I'm sure all hell will break
loose with the next major revision of Hotline.  Such is life.
...
Logs into a Hotline server opened via C<connect()>, and requests the news and
...
If EXPR evaluates to true, the C<run()> method will use blocking i/o (this
...
through the event loop more frequently (see C<event_timing()> below)
...
effect when non-blocking i/o is being used (see C<blocking()> above).  If
...
the path separator in both cases.  Note that this is the path separator
used when sending commands to the server and has no bearing on what the
path separator is on the local system.  You should not need to change this,
since all current Hotline servers use ":" regardless of the platform
they're running on.

=item xfer_bufsize BYTES

Sets the file transfer buffer size to BYTES.  Returns the current buffer
size.  The buffer size defaults to 4096 bytes when creating new
Net::Hotline::Client objects.
...
C<chat()> is a special case in that it is not treated as a "task" by Hotline.
...
failure of a C<chat()> command usually means that you have much bigger
problems.
...
Disconnects user specified by a reference to a Net::Hotline::User object
...
the user specified by a reference to a Net::Hotline::User object or a user
socket number.
...
created when C<get_file()> was called, and the download reference number
REF and the size in bytes SIZE supplied to the C<get_file()> handler
routine. When the download is I<complete>, C<recv_file()> returns either an
array (in array context) or a reference to an array (in scalar context)
...
MacBinary II file from the resource and data files.  Here's an example of
typical usage:
...
method before you have successfully retrieved the userlist from the server.
REGEX is treated as a case-sensitive anchored regular expression internally
(i.e. C</^REGEX$/>).  If your regex matches more than one user's nickname,
and C<user_by_nick()> was called in array context, an array of references
to user objects will be returned. Otherwise, a reference to the first user
...
C<default_handlers()> method.  They are enabled by default.
...
argument: a reference to the Net::Hotline::Client object itself (listed as
...
has completed.  If you want to download in the background, create your own
handler routine that calls C<fork()> or something similar.
